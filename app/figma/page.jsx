...
import EditPostModal from "./EditPostModals/page";
import AddPostModal from "./AddPostModal/page";
...
const PostPageFigma = () => {
  ...
  return (
    ...
    <div className='bg-white h-screen fixed sm:fixed md:fixed lg:fixed'>
      <div className='mt-16'>
        <h2 className='m-4 flex items-center text-gray-600 gap-1'>
          <Image className="w-4 h-4" src={home} alt="" /> Home
        </h2>
        <h2 className='m-4 flex bg-blue-600 text-white px-6 rounded-s-sm p-1'>
          <Image className="w-3 h-3" src={employee} alt="" />Employee
        </h2>
        <h2 className='m-4 flex items-center text-gray-600 gap-1'>
          <Image className="w-4 h-4" src={time} alt="" />Time Tracking
        </h2>
        <h2 className='m-4 flex items-center text-gray-600 gap-1'>
          <Image className="w-4 h-4" src={massage} alt="" />Massaging
        </h2>
        <h2 className='m-4 flex items-center text-gray-600 gap-1'>
          <Image className="w-4 h-4" src={state} alt="" />Statistics
        </h2>
        <h2 className='m-4 flex items-center text-gray-600 gap-1'>
          <Image className="w-4 h-4" src={setting} alt="" />Settings
        </h2>
        <h2 className='m-4 flex items-center text-gray-600 gap-1'>
          <Image className="w-4 h-4" src={log} alt="" />LogOut
        </h2>
      </div>
    </div>
      ...
      <div className='ml-1 mt-1 h-screen'>
        <div className="flex bg-slate-200 h-10 justify-between mb-0">
          ...
          <div className="flex items-center justify-end">
            <button className='px-1  flex mx-2 bg-slate-200 rounded-md w-16 hover:shadow-xl hover:bg-slate-400' onClick={() => handleAddClick(posts)}>ADD<Image className="h-4 w-4 object-cover mt-1" src={add} alt="" /></button>
            <button className='px-1 m-2 flex  bg-slate-200 rounded-md w-20  hover:shadow-xl hover:bg-slate-400'>Export<Image className="h-4 w-4 mt-1 object-cover" src={exporting}lt="" /></button>
            <button
      className={`px-1 flex mr-6 bg-slate-200 rounded-md w-24 hover:shadow-xl hover:bg-slate-400 ${selectedPosts.length === 0 ? 'opacity-70 cursor-not-allowed' : ''}`}
      onClick={() => setModalVisible(true)}
      disabled={selectedPosts.length === 0}
    >
      Delete All
      <Image className="h-4 w-4 object-cover mt-1" src={deletingall} alt="Delete Icon" />
    </button>
...
{currentPosts.map((post) => (
  ...
  >
    ...
    <td className="pl-0 py-0">
      <p className="flex">
        <Image className="w-8 h-8 m-2 object-cover" src={profilep} alt="" />
    ...
    <Link href={`/figma/${post.id}`} passHref>
  <Image className="h-4 w-4 object-cover opacity-70 hover:opacity-100" src={view} alt="View icon" />
</Link>

<span className="absolute -left-1 -top-1 transform -translate-y-1/2 ml-2 w-max px-2 py-1 text-xs text-white bg-gray-700 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300">
  View
</span>
</button>
    ...
    <button className="relative group" onClick={() => handleEditClick(post)}>
      ...
      <Image className="h-4 w-4 object-cover opacity-70 hover:opacity-100" src={edit} alt="Edit icon" />
    ...
    <td>
      <button
       className="relative group" onClick={() => handleDeletePost(post.id)}>
        <Image className="h-4 w-4 object-cover opacity-70" src={deleting} alt="" />
        <span className="absolute -left-3  -top-2 transform -translate-y-1/2 ml-2 w-max px-2 py-1 text-xs text-white bg-gray-700 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      Delete
    </span>
      </button>
