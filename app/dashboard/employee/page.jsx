...
const PostPageFigma = () => {
  ...
  const shouldShowDeleteAllButton = selectedPosts.length > 0;
...
<div className='h-dvh  fixed sm:fixed md:fixed lg:fixed'>
  ...
  <>
    <button
      className={`px-1 flex mr-6 bg-slate-200 rounded-md w-24 hover:shadow-xl hover:bg-slate-400 ${shouldShowDeleteAllButton ? '' : 'hidden'}`}
...
<button
  className={`px-1 m-2 flex bg-slate-200 rounded-md w-20 hover:shadow-xl hover:bg-slate-400 ${shouldShowDeleteAllButton ? 'hidden' : ''}`}
>
  Export<Image className="h-4 w-4 mt-1 object-cover" src={exporting} alt="" />
</button>
...
{currentPosts.map((post) => (
  ...
  >
    ...
    <Link href={`/dashboard/${post.id}`} passHref>
